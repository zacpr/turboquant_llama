...
static std::string kv_cache_type_normalize(const std::string & value) {
    std::string norm;
    ...
    for (unsigned char ch : value) {
        ...
        norm.push_back((char)std::tolower(ch));
    }
    return norm;
}
...
static std::string kv_cache_type_display_name(ggml_type type) {
    switch (type) {
        case GGML_TYPE_TQ3_0: return "turbo3";
...
static ggml_type kv_cache_type_from_str(const std::string & s) {
    const std::string value_norm = kv_cache_type_normalize(s);
    for (const auto & type : kv_cache_types) {
        const std::string display_norm = kv_cache_type_normalize(kv_cache_type_display_name(type));
        const std::string ggml_norm    = kv_cache_type_normalize(ggml_type_name(type));
        if (value_norm == display_norm || value_norm == ggml_norm) {
            return type;
        }
    }
