...
static constexpr int GGML_TQ3_CODEBOOK_SIZE = 8;
static constexpr int GGML_TQ3_QS_BYTES      = 12;
static constexpr float GGML_TQ3_FWHT_INV_SQRT = 0.1767766952966369f; // 1 / sqrt(32)

#define GGML_TQ3_CENTROID_VALUES \
    -0.18839061f,               \
    -0.11813298f,               \
    -0.06658060f,               \
    -0.02160247f,               \
...
static constexpr float ggml_cuda_tq3_centroids_host[GGML_TQ3_CODEBOOK_SIZE] = {
    GGML_TQ3_CENTROID_VALUES,
};

#if defined(__CUDACC__)
static __device__ __constant__ float ggml_cuda_tq3_centroids_device[GGML_TQ3_CODEBOOK_SIZE] = {
    GGML_TQ3_CENTROID_VALUES,
};
#endif

static inline __host__ __device__ float ggml_cuda_tq3_get_centroid(int idx) {
#if defined(__CUDA_ARCH__)
    return ggml_cuda_tq3_centroids_device[idx];
#else
    return ggml_cuda_tq3_centroids_host[idx];
#endif
}

static inline __host__ __device__ void ggml_cuda_tq3_pack_codes(
        const uint8_t * GGML_RESTRICT codes, uint8_t * GGML_RESTRICT dst) {
    for (int i = 0; i < GGML_TQ3_QS_BYTES; ++i) {
        dst[i] = 0;
    }

    int bit = 0;
    for (int i = 0; i < QK_TQ3; ++i) {
        const uint32_t code = codes[i] & 0x7u;
        const int byte = bit >> 3;
        const int shift = bit & 7;
        dst[byte] |= code << shift;
        if (shift > 5 && byte + 1 < GGML_TQ3_QS_BYTES) {
            dst[byte + 1] |= code >> (8 - shift);
        }
        bit += 3;
    }
}

static inline __host__ __device__ uint8_t ggml_cuda_tq3_unpack_code(
        const uint8_t * GGML_RESTRICT src, int idx) {
    const int bit = 3 * idx;
    const int byte = bit >> 3;
    const int shift = bit & 7;

    uint16_t chunk = src[byte];
    if (byte + 1 < GGML_TQ3_QS_BYTES) {
        chunk |= (uint16_t)src[byte + 1] << 8;
    }

    return (chunk >> shift) & 0x7u;
}

static inline __host__ __device__ uint8_t ggml_cuda_tq3_find_centroid(float value) {
    float best = fabsf(value - ggml_cuda_tq3_get_centroid(0));
    uint8_t idx = 0;
    for (uint8_t c = 1; c < GGML_TQ3_CODEBOOK_SIZE; ++c) {
        const float diff = fabsf(value - ggml_cuda_tq3_get_centroid(c));
        if (diff < best) {
            best = diff;
            idx = c;
        }
...
#undef GGML_TQ3_CENTROID_VALUES

static inline __host__ __device__ void ggml_cuda_tq3_fwht32(float * v) {
    for (int len = 1; len < QK_TQ3; len <<= 1) {
        for (int i = 0; i < QK_TQ3; i += len * 2) {
            for (int j = 0; j < len; ++j) {
                const float a = v[i + j];
    ...
    for (int i = 0; i < QK_TQ3; ++i) {
        v[i] *= GGML_TQ3_FWHT_INV_SQRT;
    }
}

static __device__ __forceinline__ uint32_t ggml_cuda_tq3_load_partial_u32(const uint8_t * src, int valid) {
    uint32_t result = 0;
#pragma unroll
    for (int i = 0; i < 4; ++i) {
        if (i < valid) {
...
static __device__ __forceinline__ uint8_t ggml_cuda_tq3_get_code_from_pack(
        const uint32_t pack[3], const int bit) {
    const int word_idx = bit >> 5;
    const int bit_in_word = bit & 31;
    const uint32_t word = pack[word_idx];
    const uint32_t next = word_idx < 2 ? pack[word_idx + 1] : 0;
    const uint64_t combo = ((uint64_t) next << 32) | word;
    return (combo >> bit_in_word) & 0x7u;
}

static __device__ __forceinline__ float ggml_cuda_tq3_fwht32_warp(float v, const int lane_id) {
#pragma unroll
    for (int len = 1; len < QK_TQ3; len <<= 1) {
        const float other = __shfl_xor_sync(0xffffffff, v, len);
        if ((lane_id & len) == 0) {
            v = v + other;
        } else {
            v = other - v;
    ...
    return v * GGML_TQ3_FWHT_INV_SQRT;
}

static __device__ __forceinline__ float ggml_cuda_tq3_dequant_lane(
        const block_tq3_0 * __restrict__ blocks,
        const int64_t block_index,
        const int lane_id,
        const int64_t total_blocks) {
    const bool is_tail = block_index == (total_blocks - 1);
    const uint8_t * base = reinterpret_cast<const uint8_t *>(blocks + block_index);
    ...
    if (lane_id == 0) {
        if (!is_tail) {
            ...
            word0 = ggml_cuda_tq3_load_partial_u32(base + 0, 4);
            word1 = ggml_cuda_tq3_load_partial_u32(base + 4, 4);
            word2 = ggml_cuda_tq3_load_partial_u32(base + 8, 4);
            word3 = ggml_cuda_tq3_load_partial_u32(base + 12, 2);
        }
    }
    ...
    float scale = 0.0f;
    if (lane_id == 0) {
        const uint16_t bits = word0 & 0xFFFFu;
        scale = __half2float(__ushort_as_half(bits));
    }
    ...
    uint32_t pack[3];
    pack[0] = ((word0 >> 16) & 0xFFFFu) | ((word1 & 0xFFFFu) << 16);
    pack[1] = ((word1 >> 16) & 0xFFFFu) | ((word2 & 0xFFFFu) << 16);
    pack[2] = ((word2 >> 16) & 0xFFFFu) | ((word3 & 0xFFFFu) << 16);

    const int bit = lane_id * 3;
    const uint8_t code = ggml_cuda_tq3_get_code_from_pack(pack, bit);
    float value = ggml_cuda_tq3_get_centroid(code);
    value = ggml_cuda_tq3_fwht32_warp(value, lane_id);
    return value * scale;
}
...
struct ggml_cuda_type_traits<GGML_TYPE_TQ3_0> {
    static constexpr int qk = QK_TQ3;
    static constexpr int qr = 1;
