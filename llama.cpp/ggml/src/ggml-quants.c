...
#define GGML_TQ3_CODEBOOK_SIZE 8
#define GGML_TQ3_QS_BYTES      12

static const float ggml_tq3_centroids[GGML_TQ3_CODEBOOK_SIZE] = {
    -0.18839061f,
    -0.11813298f,
    -0.06658060f,
    -0.02160247f,
...
static inline void ggml_tq3_fwht32(float * v) {
...
static inline void ggml_tq3_pack_codes(const uint8_t * GGML_RESTRICT codes, uint8_t * GGML_RESTRICT dst) {
    memset(dst, 0, GGML_TQ3_QS_BYTES);
    int bit = 0;
    for (int i = 0; i < QK_TQ3; ++i) {
...
static inline uint8_t ggml_tq3_unpack_code(const uint8_t * GGML_RESTRICT src, int idx) {
    const int bit = 3 * idx;
    const int byte = bit >> 3;
    const int shift = bit & 7;
...
static inline uint8_t ggml_tq3_find_centroid(float value) {
    float best = fabsf(value - ggml_tq3_centroids[0]);
    uint8_t idx = 0;
    for (uint8_t c = 1; c < GGML_TQ3_CODEBOOK_SIZE; ++c) {
        const float diff = fabsf(value - ggml_tq3_centroids[c]);
        if (diff < best) {
            best = diff;
            idx = c;
        }
...
void quantize_row_tq3_0_ref(const float * GGML_RESTRICT x, block_tq3_0 * GGML_RESTRICT y, int64_t k) {
    ...
    float rotated[QK_TQ3];
    uint8_t codes[QK_TQ3];

    for (int64_t i = 0; i < nb; ++i) {
        const float * xb = x + i * QK_TQ3;

        float amax = 0.0f;
        for (int j = 0; j < QK_TQ3; ++j) {
            rotated[j] = xb[j];
            amax = MAX(amax, fabsf(rotated[j]));
        }

        if (amax == 0.0f) {
            y[i].d = GGML_FP32_TO_FP16(0.0f);
            memset(y[i].qs, 0, GGML_TQ3_QS_BYTES);
            continue;
        }

        const float inv_scale = 1.0f / amax;
        for (int j = 0; j < QK_TQ3; ++j) {
            rotated[j] *= inv_scale;
        }

        ggml_tq3_fwht32(rotated);

        for (int j = 0; j < QK_TQ3; ++j) {
            codes[j] = ggml_tq3_find_centroid(rotated[j]);
        }

        ggml_tq3_pack_codes(codes, y[i].qs);
...
void dequantize_row_tq3_0(const block_tq3_0 * GGML_RESTRICT x, float * GGML_RESTRICT y, int64_t k) {
    ...
    for (int64_t i = 0; i < nb; ++i) {
        ...
        if (scale == 0.0f) {
            memset(y, 0, QK_TQ3 * sizeof(float));
            y += QK_TQ3;
            continue;
        }

        for (int j = 0; j < QK_TQ3; ++j) {
            const uint8_t code = ggml_tq3_unpack_code(x[i].qs, j);
            rotated[j] = ggml_tq3_centroids[code];
        }
